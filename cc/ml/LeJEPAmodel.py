...
class LeJEPA(pl.LightningModule):
    ...
    def _jepa_outputs(
        ...
        _, dirty_predictor_latents = self.model.decoder(dirty_encoder_latents)
    ...
    def forward(
        ...
        student_imgs, keep_mask, clean_student_latents, teacher_latents, dirty_predictor_latents, keep_masks = self._jepa_outputs(
            imgs, keep_mask=keep_mask
        )

        distill_loss, distill_metrics = self.self_distillation_loss(
            dirty_predictor_latents,
            teacher_latents,
            keep_masks,
        )
    ...
    def feature_visualizations(
        ...
        for feat_name in self.SCALE_NAMES:
            teacher_rgb, student_rgb = self._project_pair_to_rgb(
                ...
                dirty_predictor_latents[feat_name],
            )
            error_rgb = self._error_to_rgb(
                (dirty_predictor_latents[feat_name] - teacher_latents[feat_name]).pow(2).mean(dim=1, keepdim=True)
            )
...
def train_lejepa(args):
    ...
    model = LeJEPA.load_from_checkpoint(trainer.checkpoint_callback.best_model_path)
