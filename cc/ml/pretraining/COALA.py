...
class COALA(pl.LightningModule):
    ...
    def _jepa_outputs(self, imgs: torch.Tensor, keep_mask: torch.BoolTensor | None = None):
        ...
        decoder_latents, dirty_encoder_latents, predicted_latents = self.model(student_imgs, keep_mask=keep_mask)
        coala_latents = {
            feat_name: dirty_encoder_latents[feat_name] + predicted_latents[feat_name]
    ...
    def forward(self, imgs: torch.Tensor, keep_mask: torch.BoolTensor | None = None):
        _, keep_mask, clean_student_latents, teacher_latents, coala_latents, recon, keep_masks = self._jepa_outputs(
    ...
    def feature_visualizations(self, imgs: torch.Tensor, keep_mask: torch.BoolTensor | None = None):
        student_imgs, keep_mask, _, teacher_latents, coala_latents, _, _ = self._jepa_outputs(imgs, keep_mask=keep_mask)
        visualizations = {
            "context": torch.cat(
                [
